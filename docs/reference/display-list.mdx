...
A placed image. `asset` indexes the asset table, and the pixels are the host's. Layout never decoded them, so the placement comes from a header probe and the painter is the first thing in the pipeline to see the image itself.
