...
`warnings` holds the whole run's, style compilation included. See [diagnostics](../library/diagnostics.md).
...
<Page
  demo="display-list"
  highlight="baselines"
  caption="Every text run of a page, with a rule drawn on the y the display list gave it. The rules sit on the letters rather than above them, which is what a baseline is."
/>
...
The SVG painter in `@fleuron/wasm` is one worked example. See [the preview](../wasm/preview.md).
