...
`~~struck~~` needs the `gfm` dialect, which is off by default. The [dialects](#dialects) below say how to turn it on. The built-in stylesheet draws a rule through it.
...
The reference and the note can be written in either order. A note can be written far from the paragraph that refers to it. The engine puts the note at the foot of the page its reference lands on. [The CSS subset](../css-subset.mdx#footnotes) says how to style the note and the area at the foot of the page.
...
| superscript, subscript | plain text |
| math | plain text |
