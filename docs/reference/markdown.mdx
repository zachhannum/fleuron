...
A source read whole is one chapter, so its `title:` becomes that section's `title` rather than the book's. Nothing lays a section title out yet. It is carried through the tree.
...
`assemble` numbers the whole tree, so it runs once over every section rather than once per file. Node ids are document-order over the book, and a file read on its own carries no position in it.
...
An image is a block in the vocabulary and inline in markdown. One written on a line of its own is that block. One written among prose becomes a block directly after the paragraph that held it, alt text and all. Moving it changes the page, so it warns.
...
The manuscript below uses every construct in the tables above, the ones with no counterpart included. Constructs the content tree holds are laid out. The rest is laid out as prose and reported under the page.
...
`Cache` holds a source's sections against its name and a hash of its bytes, which is what a host re-rendering on every keystroke wants. The key is the name and the hash, not a node id: ids are assigned in document order over the whole book and renumber whenever a section is added or removed.
