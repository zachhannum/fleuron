...
A link can name a heading in its own source or in another source. `target-counter()` and `target-text()` then print the page and the words of that heading. The same link works in Obsidian, with no change to the source.
...
A host that reads a vault names each source by its path from the root of the vault. A link with no source part, such as `#notes`, names the source that holds the link. A link with no `#` part, such as `[[Chapter 3]]`, names the start of the source.
...
| form | example |
|---|---|
| an id that the source writes | `#hunt` |
...
4. If an earlier heading in the same source has the slug, or the source writes it as an id, the engine adds `-2`, then `-3`, and so on.

A heading that writes an id has no slug. A heading with no letter or digit in its text has no slug either.

The text form matches the way Obsidian matches a heading. It ignores case, and it ignores the characters below in the link and in the heading:
...
A link with no source part names an id that another source writes, if its own source has no heading that matches.
...
A slug and the text of a heading are not CSS ids. A stylesheet cannot select a heading by its slug, so `#notes` in a stylesheet matches no heading in the example above. An id that a source writes, such as `{#hunt}`, is a CSS id, and a link can also name it.
