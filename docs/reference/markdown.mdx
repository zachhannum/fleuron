...
Whatever string you write in `![](…)` is the name the image is matched under. It never has to be a real URL: the host supplies the bytes under that name, the engine reads the header for the intrinsic size, and a name nobody supplies is a warning and a gap.
