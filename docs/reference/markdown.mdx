...
`Dialect::attributes` is the switch. It is on by default and off under `Dialect::common_mark()`, where a brace run is prose.
...
`Dialect::common_mark()`, `Dialect::gfm()` and `Dialect::obsidian()` are the three named combinations. `common_mark()` is the strict one: it reads frontmatter and nothing else, so a brace run is prose.
