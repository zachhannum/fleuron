...
| `[text](url)` | `link` |
| `[text]{.class}` | `span` |
| `[^label]`, with `[^label]:` elsewhere | `note`, holding the blocks of the note |
...
The note becomes a `note` inline where the reference was written, and the note holds blocks. A note of two paragraphs is two paragraphs, and a note can hold a list or a quotation. Indent the lines under the first one to keep them in the note.

The reference and the note can be written in either order, and a note can be written far from the paragraph that refers to it. The engine sets the note at the foot of the page its reference lands on. [The CSS subset](../css-subset.mdx#footnotes) says how the note and the area at the foot of the page are styled.

Two things warn. A note that no reference names is kept where it was written. A second note under one label is the note its reference takes, and the first is kept where it was written.
...
| html, a task marker | nothing. There is no prose in them |
