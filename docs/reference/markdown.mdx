...
`fleuron-markdown` reads markdown into the [content tree](content-tree.md). This page lists every construct it reads, what each one becomes, and which of them warn.

Two calls do the reading. `to_sections` reads one source, and `assemble` composes sources into a book:
...
A section opens a new page. `Options::sections` sets where one begins.
...
An image is a block in the content tree and an inline in markdown. One written on a line of its own is that block. One written among prose becomes a block directly after the paragraph it appeared in, alt text and all. Moving it changes the page, so it warns.
...
A line holding nothing but a brace run names the block written under it. It takes any number of classes and at most one id, in any order:
...
An attribute line with no block under it names nothing, because the source ends under it or another attribute line follows it. It is laid out as the prose it was read as, and it warns. So is a brace run that is not classes and an id: `{key=value}` is prose and a warning, like every other construct the content tree has no node for.
...
A line wrapped in the source is a space in the tree, so where the manuscript wraps has no effect on where the lines break.

## What warns

The manuscript below uses every construct in the tables above, including the ones with no counterpart. Constructs the content tree has a node for are laid out as that node. The rest is laid out as prose and reported under the page.
...
The constructs below have no counterpart in the content tree. The frontend lays each one out as prose and reports the file, line, and column it was written at. Prose is never dropped.
...
`Options::dialect` is a set of switches, so a host's departures from CommonMark are configuration rather than a fork of the parser.
...
`Dialect::fleuron()`, `Dialect::common_mark()`, `Dialect::gfm()` and `Dialect::obsidian()` are the four named combinations. `fleuron()` is the default, and the other three are named after whose markdown they read. `common_mark()` is the strict one: frontmatter and nothing else, so a brace run is prose.
