...
A line that names nothing is set as the prose it was read as, and warns: the source ends under it, or another attribute line follows it. So is a brace run that is not classes and an id: `{key=value}` is prose and a warning, like every other construct the vocabulary has no room for.
