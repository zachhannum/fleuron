...
A stylesheet reaches a kind of element by its name: every `img`, every `blockquote`. A class or an id names one of them instead, this plate rather than every plate.
...
Two blocks also take the run written after them. A heading takes it at the end of its line, and an image takes it directly after the image when the image is alone on its line:
...
Nothing else takes a trailing run. On a blockquote it lands inside the quote's last paragraph and belongs to either. After `---` it stops the break from being one.

A line that names nothing, because the source ends under it or another attribute line follows it, is set as the prose it was read as, and warns. So is a brace run that is not classes and an id: `{key=value}` is prose and a warning, like every other construct the vocabulary has no room for.
