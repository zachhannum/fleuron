...
A heading that does not write an id gets a default id from its text. A link can then name a chapter even when its source names nothing. The engine makes the id in three steps:

1. It lowercases the text of the heading, without the markup.
2. It changes each run of characters that are not letters or digits to one hyphen, and removes a hyphen at either end.
3. If the id is taken, it adds `-2`, then `-3`, and so on, until the id is free.

An id is taken when a heading before it has the id, or when a source writes the id anywhere in the book. So an id that a source writes always wins. The count runs over the whole book in reading order, not over one source. A heading with no letter or digit in its text gets no default id. The engine does not warn about two headings with the same text.
...
The four headings get the ids `chapter-one`, `hunt`, `chapter-one-2` and `hunt-2`, in that order. A link to `#chapter-one-2` names the second chapter, and `target-counter(attr(href url), page)` prints the page that chapter starts on.
