...
A heading that does not write an id gets a default id from its text. A link can then name a chapter even when its source names nothing. The engine makes the id in four steps:
...
4. If the id is taken, it adds `-2`, then `-3`, and so on, until the id is free.

An id is taken when an earlier heading has it, or when a source writes it anywhere in the book. The engine never changes an id that a source writes. The numbers run over the whole book in reading order, not source by source. A heading with no letter or digit in its text gets no default id. The engine does not warn about two headings with the same text.
