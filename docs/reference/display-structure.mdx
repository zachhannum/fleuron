...
`navigation` holds the outline of the headings. The PDF writer reads it. [The wire](../wasm/wire.md) does not carry it. The links are on the pages.
...
The text of `::before` and `::after` has an `origin`. See [ids of pseudo-elements](#ids-of-pseudo-elements).
...
A run shaped with nothing beyond the features the shaper turns on for every run carries no set. A book is set in a handful of sets and holds hundreds of thousands of runs, so the runs share the set they carry. `Features::small_caps` and `Features::settings` read one whether it is there or not.

`small_caps` is the face's own small capitals, which `font-variant-caps: small-caps` asks for. The tag is `smcp`.

`settings` is what the stylesheet asked for, in the order the shaper read them. `tag` is a four-character OpenType feature tag, and `value` is what the feature is set to: 0 is off, 1 is on, and a higher number is the alternate of that number. A tag that appears twice takes the value of the last one. [The CSS subset](../css-subset.mdx#font-features) says which properties a stylesheet writes these with.
...
`::first-letter`, `::first-line`, `::before`, and `::after` style parts of a page that are not nodes of the content tree. [The CSS subset](../css-subset.mdx) describes them under Pseudo-elements. Each pseudo-element of an element has an id of its own.
...
| pseudo-element | runs | `origin` |
|---|---|---|
| `::first-letter` | The drop cap. | The letter in the manuscript. |
| `::first-line` | The first line of the paragraph, after the drop cap. | The text in the manuscript. |
| `::before`, `::after` | The text that the pseudo-element adds. On a block, the text is in a box. | The id of the pseudo-element, and a range in the text that it adds. |
...
| method | result |
|---|---|
| `NodeId::pseudo_element` | The element and the pseudo-element. |
| `NodeId::element` | The element that the pseudo-element belongs to. |
| `Session::inspect` | The rules that match the pseudo-element, its computed style, and its boxes. |
| `Book::source_of` | Nothing, because no source contains the pseudo-element. |
