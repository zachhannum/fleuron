...
`assets` is the table `Image.asset` and `Background.asset` index. An entry is the url an image was named by, in the content tree or in the stylesheet, and what its header reported: pixel dimensions and resolution. That is all layout read.
...
`x`, `y`, `w`, and `h` are the box. The image is clipped to it, and nothing outside it is painted.

`tile_x`, `tile_y`, `tile_w`, and `tile_h` are where one copy of the image goes, which can reach outside the box. Layout has already resolved `background-size` and `background-position` into them.

`repeat` says whether that copy tiles. A painter steps the tile by `tile_w` across and `tile_h` down, in both directions, until the box is covered.
...
For a background, resolve `asset` the same way, clip to the box, and draw the file into the tile. Where `repeat` is true, step the tile across and down until the box is covered. A url nothing can supply paints nothing: a background sits behind the text, so a placeholder there reads as part of the page.
