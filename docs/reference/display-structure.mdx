...
`::before` and `::after` on a block add a box that is not in the manuscript. [The CSS subset](../css-subset.mdx#pseudo-elements) describes these boxes. Each box has an id of its own. The `origin` of the text in the box has that id as its `node`, and `range` indexes the text of the box.
