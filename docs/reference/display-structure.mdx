...
`items` is in paint order: by layer, and inside one layer in the order the text flows. A painter walks the list from the start and paints each item over the ones before it. A painter does not sort.

Every item names its own layer. A layer comes from `z-index` on a block, which [the CSS subset](../css-subset.mdx) describes under Layers. Two layers are the engine's own. `DrawItem::PAGE_BACKGROUND` is the art behind a page, under every layer a stylesheet can name. `DrawItem::PAGE_FURNITURE` is a page number or a running head, over every one of them. The TypeScript reader exports the same two numbers, as `PAGE_BACKGROUND` and `PAGE_FURNITURE`.
...
For each page, set up a coordinate system in points with the origin top-left, then walk `items` in order. The list is already in paint order. Paint each item over the ones before it, and do not sort by `layer`.
