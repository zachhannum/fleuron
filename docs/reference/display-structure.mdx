...
`navigation` holds the links on the pages and the outline of the headings. The PDF writer reads it. [The wire](../wasm/wire.md) does not carry it.
...
`links` has one `Link` for each line of each link in the book, in the order of the pages. `area` is the area that the text of the link covers on that line. It runs from the first glyph to the end of the last glyph, and from the ascent to the descent of the font. The text of `::before` and `::after` on the link is part of the area.

`to` is where the link goes. `Place` is the box of the element that the link names, on the page where that element starts. The box is as wide as the column. `Uri` is a url outside the book, such as `https://example.com`. A link that names nothing in the book has no `Link`, and the run warns.
