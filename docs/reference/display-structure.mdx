...
Layout decides which page a chapter starts on, so a host builds a contents page or a page range beside a chapter out of this. `Session::folios` answers the same question for a handful of nodes without handing back a page: see [sessions](../library/sessions.md).
