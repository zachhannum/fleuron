...
`sections` names the content-tree sections whose content the page holds, in the order their content appears on it. A chapter that ends mid-page is followed there by the next one opening, so the page names both. A blank leaf names none.
