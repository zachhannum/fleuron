...
The display structure is what the engine produces. Painters consume it and never re-derive layout, which is why a preview and an export cannot disagree: they are two renderings of the same numbers.
...
`assets` is the table `Image.asset` indexes. An entry is the url the content tree named the image by and what its header said it was: pixel dimensions and resolution. That is all layout ever read, and it is enough for a painter to take the index back to its own pixels.
...
A placed image. `asset` indexes `LayoutOutput::assets`, and the pixels are the host's. Layout never decoded them, so the placement comes from a header probe and the painter is the first thing in the pipeline to see the image itself.
...
For an image, resolve `asset` through `LayoutOutput::assets` and draw the file that url stands for into the box the item gives. A url nothing can supply is a box worth outlining: the space is still on the page.

The SVG painter in `@fleuron/wasm` is one worked example. See [the preview](../wasm/preview.mdx).
