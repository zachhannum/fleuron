...
Every glyph the engine draws comes from a face in a `FontRegistry`. The engine bundles one family, EB Garamond, and a stylesheet adds more with `@font-face`.
...
An `@font-face` rule names a family and the file to read it from:
...
Whatever string `url()` names is handed to a `FontLoader` unchanged. It never has to be a real URL. The following loader reads each one as a path under one directory:
...
Declaring `font-style` or `font-weight` on the `@font-face` rule overrides that, and the file registers as that one style. Use it when a file names instances you do not want, or when you are pinning one weight of a family you are assembling out of several files. The following pair assembles one family from an upright file and an italic one:
...
The engine synthesizes nothing. It never slants an upright to make an italic, and never thickens a regular to make a bold. A family with no italic is set upright, and a stack that resolves nothing falls back to the first registered face. Both warn, naming what was asked for, and the book is laid out either way. See [diagnostics](diagnostics.mdx).

## Metrics

`registry.select` resolves a family and the attributes wanted to a face, and `registry.metrics` gives that face's units per em, ascender, descender, line gap, and cap height, in font units.
