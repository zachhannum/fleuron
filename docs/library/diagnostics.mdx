...
| a font without a feature the stylesheet asked for  | The face carries none of an OpenType feature a `font-feature-settings` or a `font-variant` longhand asked for. The warning names the family and the tag, and the text is set without the feature. See [font features](../css-subset.mdx#font-features). |
