...
| a reference that names nothing                     | A `target-counter()` or a `target-text()` names a source, a heading, or an id that the book does not have. The engine warns the same way for `attr(href url)` on an element that is not a link. A web address such as `https://example.com` prints nothing and does not warn. The pseudo-element prints nothing. The engine names the line and column of the link. See [the CSS subset](../css-subset.mdx). |
