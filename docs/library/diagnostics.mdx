...
| a link that names nothing                          | A link names a source, a heading, or an id that the book does not have. The text of the link is not a link in the PDF. The engine names the line and column of the link. A link to a web address, such as `https://example.com`, is a link in the PDF, and the engine does not warn. |
