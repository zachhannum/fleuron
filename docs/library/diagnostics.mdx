...
| an image not supplied to the engine                | A url that is missing, or bytes the header probe does not recognize. The manuscript names one for every image it places, and the stylesheet names one for every `background-image`. The image is skipped, and a url the stylesheet named is warned about at the line and column it was written at.                                                                                                                                            |
| an image taller than the page                      | The image is scaled down to the content box.                                                                                                                                                                                     |
| a reference that names nothing                     | A `target-counter()` or a `target-text()` names an id that no element has, or a url that is not an id. The engine warns the same way for `attr(href url)` on an element that is not a link. The pseudo-element prints nothing. The engine names the line and column of the link. See [the CSS subset](../css-subset.mdx). |
