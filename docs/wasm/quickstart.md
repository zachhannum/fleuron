...
`Preview` starts the worker, loads the module into it, keeps the session, fetches the fonts, and paints pages into an element the host gives it. The following program mounts one and turns to page 12:
...
The same thing is available in pieces: a worker, a client that talks to it, a reader for what comes back, and a painter. Assemble them yourself to paint the pages some other way, to start the engine somewhere `Worker` is not what starts it, or to read the pages without drawing them.

### The worker

Layout belongs off the main thread. Setting a whole book is a long enough job that running it on the main thread drops interactions.

The package ships a worker at the `fleuron/worker` export, which is what `Preview` starts. A host that needs nothing else in there points `new Worker` at that path and skips to the client below.

Write a worker of your own when something else has to run in there too, or when the bundler needs to hand the module its bytes rather than let it fetch them. The following worker loads the engine and hands it whatever the host posts:
...
The host side keeps a `Client`, which pairs replies with calls and decides which renders are still worth painting. The following program starts the worker above, sets a manuscript and a stylesheet, and paints the first page:
...
The module keeps a [session](../library/sessions.md) open between calls, so the second render of a book pays for the edit rather than for the book. The following calls restyle, edit one chapter, and register a face:
...
The following call sends three layers, with the author's own sheet last:
...
`exportPdf` writes the book the preview is showing as PDF bytes:
...
Nothing about the module needs a worker. The following program lays a book out and writes a PDF on whatever thread it is called on, which is what a build step or a test wants:
...
`npm test` then runs the package's tests, and `npm run test:browser` runs the ones that need a real browser.
