...
| `svg(page?)` | the markup for a page already fetched, painted but not mounted; empty for one that is not |
...
Where `text-transform` or small capitals changed the characters, the run has both. The painter draws what was shaped, and beside it is what the author wrote, which is what selection and copy and paste return.
