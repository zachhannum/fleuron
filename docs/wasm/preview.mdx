...
| `ink` | `'#000000'` | The fill for a run or a rule left in black. A run the sheet coloured takes its own colour, so an `ink` other than black draws only what the sheet left black, and those runs are black in the export. |
