...
Those files have to reach the browser as `FontFace`s. The host already holds the bytes for a face it registered. The face built into the engine has no URL to fetch it from, so the module hands it back:
