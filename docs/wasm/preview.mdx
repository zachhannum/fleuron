...
| `ink` | `'#000000'` | The fill for a run or a rule the sheet left black. A coloured run takes its own colour instead. `ink` reaches the preview only: the export prints black wherever the sheet named no colour. |
