...
So the painter leaves the browser nothing to work out. Each `<text>` sets an x for every one of its characters, taken from the glyph the engine placed there.
