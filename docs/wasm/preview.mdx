...
The engine's output is a display structure: a book's worth of glyph positions, rules, and image boxes. Turning that into pixels is a painter's job, and the package ships one. It draws a page as SVG, in two layers. The lower layer is the glyphs, one `<text>` element per run, with an explicit x for every character. The upper layer is invisible, one `<text>` per line, and it is what a reader selects and copies from.

This page covers mounting the preview, the methods that drive it, and calling the painter directly.

## Mounting

The following program mounts a preview, sets a manuscript and a stylesheet, and turns to page 12 at one and a half times size:
...
Nothing in the package fetches a url. The host fetches the file and hands over the bytes, under the same string the manuscript wrote. The following program supplies one image at mount and another after the book is already on screen:
...
| `svg(page?)` | the markup for a held page, painted but not mounted; empty for one that is not |
...
`setBook` takes the sources in reading order. The following call sets a book of two chapters:
...
Naming a book costs no layout, since the pages already on screen are the pages the export writes under the new name. `extra.language` is the one field a stage below the PDF writer reads, and a book that changes it is hyphenated again.
...
Nothing is interrupted to make that happen. A render occupies the worker from start to finish, so the collapsing happens in the gap before one begins. A render already running finishes, its output is dropped as stale, and the next one runs. A superseded render is one that never started rather than one abandoned halfway, which is what keeps the session's caches sound.
...
`Preview` fetches one page at a time, not the whole book: the page on screen and the one either side of it. Assigning to `page` inside that reach paints at once, from what is already held. A page further off asks the worker and paints once the reply lands, the frame staying as it was until then. `pages` reports how many pages the book has regardless of how much of it has crossed.

An edit drops whatever pages were held, since the book they came from no longer stands, and fetches the page on screen fresh against the book the edit produced.

`onRender` is called after every render that reaches the screen, with the reply behind the page on screen. `output.pages` holds that page alone, `output.bookPages` is the book's own length, and `output.fonts`, `output.assets` and `output.warnings` are the whole run's.
...
`fleuron-react` is the same preview as a component:
...
The manuscript and the stylesheet are props, so an edit is a re-render and the engine is handed the one input that changed. `images` is a prop too, so an image that arrives after the book does still reaches the page. `onMount` hands back the underlying preview, for the page count and the PDF export.
...
Call the painter directly for the pages without the mounting, when drawing to a canvas, running a scroll container of your own, or drawing where there is no DOM. The following program paints one page at twice its size:
...
| `ink` | `'#000000'` | The fill for a run or a rule the stylesheet left black. A run with a color of its own takes that instead. `ink` reaches the preview only, since the export prints black wherever the stylesheet named no color. |
...
A browser handed a string and a font does its own typesetting: it chooses the glyphs, applies kerning and ligatures, and works out where each one goes. Fleuron has already done that work. If the browser did it again the two answers would differ a little, over a different shaper version, a rounded advance, or a substituted font.
...
Where `text-transform` or small capitals changed the characters, the run carries both. The painter draws what was shaped, and beside it is what the author wrote, which is what selection and copy and paste return.
...
Those files have to reach the browser as `FontFace`s. The host already has the bytes for a face it registered. The face built into the engine has no URL to fetch it from, so the module hands the bytes back. The following program registers one face with the browser:
...
One file can answer for several faces. The bundled EB Garamond is a variable font whose Regular, Medium, Bold and the rest are one file read at different points on a `wght` axis, and each of those has its own `fontId`. The font table records the point, and the painter writes it out as `font-variation-settings`. A painter that ignored it would draw every weight at the file's default.
