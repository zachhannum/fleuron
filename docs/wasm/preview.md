...
Layout never decodes an image. It probes the header for the intrinsic size, reserves a box of the right shape, and records an index into a table of assets the host supplied. `asset` is where you turn that index back into something a browser can load: any URL an `<img>` would take, including a `blob:` or `data:` one, or `null` if you cannot supply it. On `null` the painter outlines the box the image would have filled, so the space it takes is still visible.

There is no op for handing assets to the module yet, so a display list from `@fleuron/wasm` never carries an image. `asset` matters only to a painter given pages from somewhere else.
