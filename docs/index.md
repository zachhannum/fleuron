...
**A Rust library.** `fleuron` is the engine: style compilation, box construction, inline layout, fragmentation, page assembly. Pure library, no I/O. One call lays a book out, and a [session](library/sessions.md) holds the stages between edits for a preview that re-runs only what changed. Start at the [library quickstart](library/quickstart.md).
