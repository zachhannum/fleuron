...
`::before` adds text before an inline element, and `::after` adds text after it. An inline element is a link, emphasis, strong text, or code. The `content` property holds the text. The text properties style it. A rule with no `content` adds nothing. If the rule selects a block, the engine warns and adds nothing. See [Cross-references](#cross-references) for how `content` prints the page number or the text of another element.
...
| `relative` | The engine draws the block moved by its insets. Nothing around it moves. |
...
With `position: relative`, the engine moves the block up for a negative `top` and down for a positive `top`. It moves the block right for a positive `left`. Where both insets of an axis are set, the engine uses `top` or `left` and ignores the other one.
...
The insets also decide the width of the block. If both `left` and `right` are set, the block fills the space between them. If only one is set, the block fills the space from that inset to the far edge of the content box. If neither is set, the block is as wide as a line of text. The lines of the block break to that width.

`wrap-flow` and `shape-outside` work on a block as they work on an image. A block has no alpha channel, so `shape-outside: auto` wraps the text around the box of the block.
...
The target must have an id. In markdown, you write the id of a heading in braces after its text. You write the id in a link after a `#`. See [the markdown mapping](reference/markdown.mdx) for the other ways to give an element an id.
...
- The target. `attr(href url)` is the url of the link. A string such as `"#the-hunter"` names the same target for every element that the rule selects.
...
A reference prints only in `::before` and `::after` on an inline element. If no element has the id that a reference names, the pseudo-element prints nothing. The engine warns and names the line and column of the link. The engine does the same for a url that is not an id, such as `https://example.com`. It also does the same for `attr(href url)` on an element that is not a link.
...
| generated content | `::before` and `::after` on a block, `target-counters()`, the second argument of `target-text()`, and `attr()` outside a reference |
