...
```
book section h1 h2 h3 h4 h5 h6 p blockquote hr img code em
strong a
```
...
`position: absolute` takes the image out of the text. The insets say where on the page it goes. `top`, `right`, `bottom`, and `left` measure from the content box, which is the space inside the page margins. A negative inset reaches into the margin. An inset of `auto` lets the opposite inset place the image. Where both insets of an axis are `auto`, the image sits at the left or the top edge of the content box.

The image keeps the size its own file gives it. The engine scales it down where that size does not fit the content box. The margins of the image are the space it keeps clear of the text.

`wrap-flow` says which side of the image the text wraps on.
...
This example puts an image in the top right corner of the page it lands on. The text of that page wraps down the left side of the image.
...
`shape-outside` sets the text around the shape of an image rather than around its box. A contour is the outline the text keeps clear of. It applies to an image the sheet has put against the page and given a `wrap-flow` other than `auto`, because that is the only place text sets beside an image.

| value | the outline the text keeps clear of |
|---|---|
| `none` | the box, which is the default |
| `auto` | the shape the image's own alpha channel covers |
| `polygon(…)` | the points you write |

`polygon()` takes pairs of coordinates, one pair to a point, separated by commas. Each pair is across the box and then down it. A percentage measures against the box, and the polygon closes itself. Three points are the fewest that enclose anything.
...
`auto` reads the shape from the file. The engine decodes the image to do it, and it decodes no image that no rule asks it to. An image the sheet leaves among the paragraphs is never decoded, whatever `shape-outside` says about it. PNG, GIF and WebP carry an alpha channel. JPEG does not, and an image the engine finds no alpha in keeps the text clear of its box.

`shape-margin` holds the text off the contour by a distance. The engine reads the contour over the band the margin grows, so a sloping edge holds the text off by at least the distance you ask for.
...
The engine reads a contour one line-height at a time. A line is either set beside the image or clear of it. Where the contour leaves a band clear across the whole image, the text of that band runs the full width.
...
|---|---|
| layout | floats, tables, grid, flexbox, transforms |
...
| contours | `shape-outside: circle()`, `ellipse()`, `inset()`, `path()`, `url()` and the `<box>` keywords, `shape-image-threshold`, and a contour on an element the sheet has not taken out of the flow |
...
| columns | `column-fill`, `column-rule-color`, `column-span`, the `columns` and `column-rule` shorthands, and columns on any element |

Three of those have visible consequences today. The last page fills its columns in order and stops short rather than balancing them. A column rule uses the book's text color. Nothing can run the full width with columns above and below it.
