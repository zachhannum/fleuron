...
The image keeps the size its own file gives it. The engine makes it smaller where that size does not fit the content box. The margins of the image hold the text away from it.
...
`shape-outside` wraps the text around the shape of an image instead of around its box. That shape is a contour: the outline the text goes around.

A contour works only on an image you put against the page and give a `wrap-flow` other than `auto`. Text goes beside an image only there.
...
`auto` reads the shape from the file. The engine decodes the image to do it. It decodes no image that no rule asks it to, and it never decodes an image you leave among the paragraphs.

PNG, GIF and WebP carry an alpha channel. JPEG does not. Where the engine finds no alpha channel, the text goes around the box and the engine warns you.

`shape-margin` holds the text away from the contour. The margins of the image do not. A contour replaces the box those margins surround, so use `shape-margin` where you want a gap between the image and the text.

The engine reads the contour over a band that `shape-margin` makes taller. A sloping edge therefore holds the text away by at least the distance you ask for.

The following example wraps the text around the shape the image's own alpha channel covers, and keeps 9pt between the two:
...
The engine reads a contour one line-height at a time. A line goes either beside the image or below it. Where the contour leaves a band empty across the whole image, the text of that band runs the full width.
