...
A page carries a background: a color, an image, or a color with an image over it. The engine paints it over the whole page box, margins included, before anything else on the page. The text, the rules, and the page numbers are set over it.
...
The four background properties are described under [backgrounds](#backgrounds). They mean the same thing on a page as on a block. The box they measure against is the page box rather than the border box.

The engine opens no files of its own. Whatever string `url()` holds is the name the host resolves to bytes, the same way `@font-face` resolves a face. A url nothing resolves warns at the line and column it was written at, and the page is set without the image.
...
A background is a color, an image, or a color with an image over it. The engine paints it over the block's border box, and the border draws over it. A block broken across a page turn paints its background on each page, over the part of the block that page holds.
...
`background-size: cover` is the smallest size that covers the box, so the box crops what runs past it. `contain` is the largest size that fits inside the box, so the box keeps the remainder. One length sizes the image across the box, and the image's own proportions size it down the box. A percentage measures against the box.
...
The engine opens no files of its own. Whatever string `url()` holds is the name the host resolves to bytes. A url nothing resolves warns at the line and column it was written at, and the block is set without the image.
