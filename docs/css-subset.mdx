...
`shape-outside` sets the text around the shape of an image rather than around its box. A contour is the outline the text keeps clear of.
...
`auto` reads the shape from the file. The engine decodes the image to do it, and it decodes no image that no rule asks it to. PNG, GIF and WebP carry an alpha channel. JPEG does not, and an image the engine finds no alpha in keeps the text clear of its box.
