...
```
book section h1 h2 h3 h4 h5 h6 p blockquote hr img ul ol li
table thead tbody tr th td code em strong a
```
...
On an inline element, `::before` adds text before the element, and `::after` adds text after it. An inline element is a link, emphasis, strong text, or code. The text properties style that text.
...
`position: absolute` puts the box [against the page](#putting-a-block-against-the-page), the same as a block. The place of the box in its element decides the page. The box then takes up no room among the paragraphs. With `wrap-flow`, the text wraps around the box.
...
A custom property that depends on itself has no value, such as `--a: var(--b)` with `--b: var(--a)`. The engine warns about each custom property in the cycle. If the value that `var()` gives is not one the property takes, the engine warns about an unsupported value. The property then takes its inherited or initial value.
...
`top`, `right`, `bottom`, and `left` are the insets. Each one takes a length or a percentage. A percentage on `left` or `right` is a percentage of the width of the content box. A percentage on `top` or `bottom` is a percentage of its height.
...
| positioning | `position: fixed` and `position: sticky`, the `inset` shorthand, `wrap-flow: clear`, `minimum` and `maximum`, and a width or a height on a positioned block |
...
| inline boxes | a border, padding, or background on an inline element, and a border or padding on `@page` |
