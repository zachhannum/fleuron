...
|---|---|
| layout | floats, grid, flexbox, transforms, `max-height`, `align-content` on a block, and the `align-content` values other than `start`, `center`, and `end` |
...
| positioning | `position: fixed` and `position: sticky`, the `inset` shorthand, `wrap-flow: clear`, `minimum` and `maximum`, and a width or a height on a positioned box |
