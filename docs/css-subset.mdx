...
A value of `1` or `on` turns a feature on, and a value of `0` or `off` turns it off. A tag written on its own is on. A feature that draws more than one alternate takes the number of the alternate.
...
The engine turns some features on for every run of text. [The fonts page](library/fonts.md#font-features) lists them. `font-feature-settings: "liga" 0` turns one of them off.
...
font-variant-caps letter-spacing text-transform
...
The following example opens a chapter on an italic line:
...
`font-family`, `font-style`, and `font-weight` are separate properties, and a rule can set one of them alone. A word inside the line that carries a weight of its own keeps that weight. An italic first line over a bold word sets that word in the bold italic.
