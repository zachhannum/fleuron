...
The engine paints every block in a layer. It paints a higher layer over a lower one. `z-index` names the layer of a block. Inside one layer the engine paints the blocks in the order they are written in.
...
Two layers are the engine's own. A block reaches neither. The engine paints the art behind a page under every layer a stylesheet can name. The text of a page covers that art with no `z-index` in the stylesheet. The engine paints a page number and a running head over every layer, so a border against the page never covers them.

`z-index` here is one number in one order over the whole page. In CSS, `z-index` on a positioned block also starts a stacking context, which is a paint order of its own. The layers of the blocks inside that block then count against it rather than against the page. A book is flat, so the engine reads the number and starts no context. `position`, a background image, and a page break leave the order alone.
