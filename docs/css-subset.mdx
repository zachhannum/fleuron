...
| generated content | `::before` and `::after` on the book, a table row, a table cell, `thead`, or `tbody`, `position: absolute` on a pseudo-element, `target-counters()`, the second argument of `target-text()`, and `attr()` outside a reference |
