...
A book with two columns often puts its headings and its tables across both of them. Set `column-span: all` on a block to put it across the whole content box, with columns above it and columns below it.

The following example puts every chapter heading and every table across both columns:
...
The engine does not balance the columns above a spanning block, so they can end at different heights. The text below the block starts under it. That text fills the first column and then the next. A column rule stops above a spanning block and starts again under it.

If a spanning block does not fit in the space left on the page, it moves to the next page whole. Only a spanning block that is taller than a page splits across pages. The built-in stylesheet gives every heading `break-after: avoid`. If the text under a spanning heading does not fit on the page, the heading moves to the next page with that text.

`column-span` applies to the blocks of a section. It does not apply to a block inside a quotation or a table cell. On a page with one column, `column-span` has no effect.
