...
```
book section notes note h1 h2 h3 h4 h5 h6 p blockquote pre
hr img ul ol li table thead tbody tr th td code em strong a
s span
```
...
text-decoration:           none | [ underline || overline || line-through ] || solid || double || <color> || <length>;
...
A font can carry OpenType features: alternate glyphs that a four-character tag names. Old-style figures, a stylistic set, and a discretionary ligature are all features. `font-feature-settings` asks a font for one by its tag.

```css
h2 { font-feature-settings: "ss01" 1 }
```

A value of `1` or `on` asks for a feature. A value of `0` or `off` turns one off. A tag written on its own is on. A feature that draws several alternates takes the number of the alternate.

Three properties name a feature without spelling its tag:
...
Each ligature value has a `no-` form that turns its feature off, such as `no-common-ligatures`. `font-variant-ligatures: none` turns off all four.

The engine reads the named forms first and `font-feature-settings` after them. A tag that both name takes the value the stylesheet spelled.

The shaper turns some features on for every run. [The fonts page](library/fonts.md#font-features) lists them. `font-feature-settings: "liga" 0` turns one of them off.

A feature changes which glyphs a run is drawn from, so it changes the width of the run. The engine measures the glyphs it shaped, and the lines break against that width.

For a font that carries none of a feature the stylesheet asked for, the engine warns and names the font. The text is set without the feature. Nothing is synthesized.
...
The built-in stylesheet draws a rule through `s`, which is what `~~struck~~` in the [manuscript](reference/markdown.mdx#inlines) becomes. The following example underlines a cross-reference in gray and draws a double rule through a deleted passage:
...
An inline element is a link, emphasis, strong text, struck text, code, or a span. It can take a background, padding, and a border, the way a block does. The properties are the ones in [margins, borders, and padding](#margins-borders-and-padding), [backgrounds](#backgrounds), and [rounded corners](#rounded-corners). A margin on an inline element is not in the subset.
...
A code block holds text rather than an inline `code`, so `pre code` selects nothing. `code` is the inline code span, and a rule on it reaches code written among prose.
...
The engine does no syntax highlighting. The word after the opening fence reaches the content tree as `info`, for a painter that reads it. See [the markdown mapping](reference/markdown.mdx#code-blocks).
...
A footnote is written in the manuscript where its reference belongs. The engine puts the note at the foot of the page that reference lands on. [The markdown mapping](reference/markdown.mdx#footnotes) says how to write one.
...
The area takes the properties under [Blocks](#blocks): a margin, a border, padding, and a background. It is as wide as a column, which on a page that does not divide is the whole content box. Its height is what the notes in it come to.
...
The engine prints the number twice: as the reference in the line, and as a mark beside the note. `list-style-type` on the note spells both, in the [values a list marker takes](#lists). The mark carries a point and a space after it, as a list marker does. It hangs in the padding to the left of the note, so `padding-left` is the indent it hangs in.

The engine sets the reference in the font's superior figures, which is the `sups` [feature](#font-features). It asks for the feature on the reference alone, so the figures in the prose of the note are the ordinary ones. The reference stands on the baseline of the line it is written in, at the size `note` gives it. For a font that carries no superior figures, the engine warns and names the font, and the reference is set in the ordinary figures.
...
Which words fall on that line depends on the style applied to it. `font-size`, `font-variant-caps`, `letter-spacing` and `text-transform` change the width of the text, which moves the break. The style then applies to whatever words end up on that line.
...
/* The notes of a page stand at the foot of its content box, under a
   rule. */
notes {
  margin-top: 1em;
  padding-top: 0.4em;
...
/* A note is set smaller than the text, and starts flush in the
   letters the book is set in, wherever its reference was written. The
   number is the reference in the line and the mark beside the note,
   and it hangs in the indent to the left of the note. */
note {
  font-size: 9pt;
  line-height: 1.2;
  font-style: normal;
...
| footnotes | `::footnote-call` and `::footnote-marker`, a note set in the margin or at the end of the book, `float: footnote`, a rule that reaches the area of one page rather than every page, a note inside a block against the page, and a raised reference from a font with no superior figures, which wants `vertical-align` |
