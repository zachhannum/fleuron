...
| `-d`, `--dialect` | Which markdown is being read: `commonmark`, `gfm` or `obsidian`. The default reads attribute lines; under `commonmark` a brace run is prose. See [the markdown mapping](../reference/markdown.mdx). |
